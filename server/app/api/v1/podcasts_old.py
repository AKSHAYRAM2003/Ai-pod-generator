...
async def check_rate_limit(user_id: int, db: AsyncSession):
    ...
    if len(podcasts) >= MAX_PODCASTS_PER_DAY:
        raise HTTPException(
...
async def create_podcast(
    ...
    podcast = Podcast(
        ...
        status=PodcastStatusEnum.PENDING,
        is_public=False
    )
    ...
    task = generate_podcast_task.delay(podcast.id)
    
    # Store task_id for status tracking (optional, for future enhancement)
    # podcast.task_id = task.id
    # await db.commit()
...
async def get_my_podcasts(
    ...
    # Apply pagination
    podcasts = query.order_by(desc(Podcast.created_at)).offset(
        (page - 1) * page_size
    ).limit(page_size).all()
    
    # Load categories
    for podcast in podcasts:
        podcast.category = db.query(Category).filter(Category.id == podcast.category_id).first()
...
async def get_podcast_status(
    ...
    progress = None
    if podcast.status == PodcastStatusEnum.PENDING:
        progress = 0
    elif podcast.status == PodcastStatusEnum.PROCESSING:
        # Could be enhanced with more granular progress from Celery
        progress = 50
